...
void Signal::generateFullSignal(float startTime, float endTime, int length){
    /* NOTE: PROBABLY NEED A NEW FUNCTION FOR FUNCTIONALITY DESCRIBED IN NOTE... AS OF NOW, STARTTIME AND ENDTIME ARE BEING USED AS INDICES, AND NOT AS ACTUAL TIME PARAMETERS */
...
/* NOTE: need to find a better way to deal with signal delay...*/
float Signal::getValueAtIndex(int index){
    if (index >= signalDelay && (index-signalDelay) < signalLength){
        return signal[index-signalDelay];
    }
    return 0.f;
}
