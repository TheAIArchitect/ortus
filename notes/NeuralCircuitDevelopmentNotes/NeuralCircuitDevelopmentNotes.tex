...
\begin{outline}
    \point Location--based synaptic growth and decay
        ...
        \subpoint then each synapse will ``look around'' by looking at near by synapses, and either probabalistically attempting to create a synapse, or (and probably better), creating a synapse if one near by (from the same neuron) is sufficiently strong (perhaps 2 at 1/2 of the required strength would be fine too?)
...
From there, the next set of goals revolve around the implementation of a very simple (e.g. 16 ``pixel'') visual system, that will enable ortus to ``see'', and associate visual stimuli with emotion. After the ``Initital Goals'', during which we will have added fear and happy centers, we might add anger and sad centers.
...
\point Article talking about synaptic strength scaling with connections (more or less inversely proprotional), seems to further suggest that it's alright to model regions of the brain with one or a few neurons, and expand as necessary for more nuanced behavior.
\end{outline}
...
\begin{outline}
    ...
    \point Cortico--amygdala: plasticity may be induced either pre or post synaptically, but is mediated primarly by increases in presynaptic NT release.
...
\begin{outline}
   \point Emotions, motivations, and reinforcement lare a closely related, evolutionarily--conserved phenomena maintaining the integrity of an individual and promoting survival in a natural environment. 
\end{outline}
