...
void DataSteward::init(){
    ...
    NUM_ELEMS = bioElements.size();
    NUM_ROWS = NUM_ELEMS;
    LEN = NUM_ROWS * NUM_ELEMS;
    // set the mass points in the bioElements, and also keep separate references to the mass points for easy access
...
void DataSteward::addElementInfoToModules(){
    for (int k = 0; k < bioElements.size(); k++){
        NeuronInfoModule* tnimp = (NeuronInfoModule*) bioElements[k];
...
void DataSteward::populate_cs_gj_matricies(float **cs_matrix, float **gj_matrix, unordered_map<string,int>& neuron_names_to_index_map, size_t num_rows, size_t num_cols){
    ...
    int numElems = bioElements.size();
    for (int i = 0; i < numElems; ++i){
        int numConns = bioElements[i]->out_conns.size();
        std::vector<Connection> tempConns = bioElements[i]->out_conns;
 ...
 * 
 * NOTE NOTE: this needs to be redone using the ElementInfoModules (bioElements)
 */
...
unordered_map<string, int> DataSteward::map_name_to_id(){
    ...
    for(int i = 0; i < numElems; ++i){
        ...
        string name = bioElements[i]->name;
        ...
        if(it == name_to_id_map.end()){
            ...
            name_to_id_map[name] = bioElements[i]->element_id;
        }
        else{
            printf("Why do you have duplicates in the bioElements array? '%s' exists at least twice... \n",name.c_str());
            exit(82);
        }
    }
...
void DataSteward::makeNeuronModules(){
    ...
    for (int i = 0; i < num_elements; i++){
        if (bioElements[i]->getEType() != MUSCLE){
            ++num_neurons;
        }
    }
...
void DataSteward::makeMuscleModules(){
    ...
    for (i = 0; i < num_elements; i++){
        if (bioElements[i]->getEType() == MUSCLE && !bioElements[i]->ablated){
            temp_mim[muscle_count] = (MuscleInfoModule*) bioElements[i];
            muscle_count++;
        }
    }
    /*
