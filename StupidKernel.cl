 ...
 */
__kernel void StupidKernel(__global float *voltages_matrix,
...
//    v_decay_constant = .205f;// it keeps 90% of its charge each time step. maybe it should keep more???
    v_decay_constant = .15f;// it keeps 90% of its charge each time step. maybe it should keep more???
    ...
    if(gid <= (num_rows_in_matrix - 1)){
        ...
        for(loop_iter = 0; loop_iter < num_cols_in_matrix; ++loop_iter){
            ...
            if (their_v_curr < thresh){ // nothing happens if it's less than thresh.
                continue;
            }
            ...
            if(cs_weight != 0.0){
                ...
                conductance = 1.0 - (1.0/(1.0 + exp(-5.0*(their_v_curr/100.0))));
                added_v = cs_weight * conductance * their_v_curr;
                
                if(my_polarity != 1){ // if not excite
                    added_v = -1 * added_v;
                }
                if (shouldProbe == 1){
            ...
            if(gj_weight != 0.0){
                if (their_v_curr < my_v_current){ // while GJ's are bidirectional, we'll let the 'pre' deal with this when it is a 'post' and sees this connection.
                    continue;
                }
