...
//  pelegans
//
//  Created by Andrew W.E. McDonald     --> All code
//  Created by Sean Grimes on 11/21/15. --> Class structure
//  Copyright © 2015. All rights reserved.
//

#include "include/CLHelperClass.hpp"

bool CLHelperClass::setup_opencl(){
    
    int device_to_use = 0;
//    err = clGetDeviceIDs(NULL, CL_DEVICE_TYPE_GPU, 2, device_ids, &num_devices);
//    check_and_print_cl_err(err);
/*
    num_devices = 0;
    err = clGetDeviceIDs(NULL, CL_DEVICE_TYPE_ALL, 1, device_ids, &num_devices);
    fprintf(stderr, "num_devices: %d\n", num_devices);
    check_and_print_cl_err(err);
    
//    else if(device_ids[1] != nullptr){ // if we have at least one GPU, do we have 2?
//        fprintf(stderr, "Machine has 2 GPU Devices!!!\n");
//        device_to_use = 1;
//#ifdef LOG_DEBUG
//        log_.error("Machine has two GPU Devices, setting to second device", FUNC);
//#endif
//    }   
...
*/  
    ...
    check_and_print_cl_err(clGetPlatformIDs(1, &platform_id, &num_platforms));
    check_and_print_cl_err(clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_DEFAULT, 1, &device_id, &num_devices));
    //fprintf(stderr, "num_devices: %u\n");
    //device_id = device_ids[device_to_use];
    
    print_selected_device_info(device_id, 0);
    
    // Create a compute context
    context = clCreateContext(0, 1, &device_id, NULL, NULL, &err);
    /** NOTE NOTE HACK HACK (due to my failing integrated GPU, i think)...
     Adding this error check in so that if we tried using device 2 and it failed, we'll just go with 1...
     **/
    if (err != CL_SUCCESS){
        device_to_use = 0;
        device_id = device_ids[device_to_use];
        context = clCreateContext(0, 1, &device_id, NULL, NULL, &err);
#ifdef LOG_DEBUG
        log_.error("FAILED TO USE SECOND GPU -- falling back to first.", FUNC);
#endif
    }
    /** NOTE NOTE -- END HACK **/
    check_and_print_cl_err(err);
...
void CLHelperClass::read_kernels_from_file(const char *fname, char **programBuffer){
...
void CLHelperClass::check_and_print_cl_err(int err){
    if (err == CL_SUCCESS) return;
    
    std::string err_msg = get_opencl_error(err);
    std::cout << err_msg << std::endl;
#ifdef LOG_DEBUG
    log_.fatal("CLHelper: " + err_msg, FUNC);
#endif
    throw std::runtime_error(err_msg);
}

void CLHelperClass::check_and_print_cl_program_build_err(int err, cl_program *programp){
    if (err != CL_SUCCESS){
        size_t len;
        char buffer[2048];
#ifdef LOG_DEBUG
        log_.fatal("Failed to build program", FUNC);
#endif
        clGetProgramBuildInfo(*programp, device_id, CL_PROGRAM_BUILD_LOG, sizeof(buffer), buffer, &len);
#ifdef LOG_DEBUG
        log_.fatal(std::string(buffer), FUNC);
#endif
        throw std::runtime_error("Couldn't build executable");
    }
}

void CLHelperClass::print_selected_device_info(cl_device_id cdi, int device_num){
...
std::string CLHelperClass::get_opencl_error(int err){
...
void CLHelperClass::print_dferr(const char *opt){
    std::string dferr = "\
    #########################################################################################################################################################################\n\
    #                                                                                                                                                                       #\n\
    #                                                                                                                                                                       #\n\
