...
__kernel void NewKernel(__global float *voltages_matrix,
    ...
    v_decay_constant = .1f;// it keeps 90% of its charge each time step. maybe it should keep more???
    ...
    if(gid <= (num_rows_in_matrix - 1)){
        ...
        float v_decay = 0;//get_v_decay(my_v_current, v_decay_constant, v_init);
        size_t voltage_idx;
        size_t weight_idx;
        voltage_idx = 0;
        weight_idx = 0;
